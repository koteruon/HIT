...
class PoseTransformerV2(nn.Module):
    def __init__(
        ...
        self.num_frame_kept = num_frame
        self.num_coeff_kept = (
            self.num_frame_kept
        )  # args.number_of_kept_coeffs if args.number_of_kept_coeffs else self.num_frame_kept
    ...
    def forward(self, x):
        ...
        x = torch.cat(
            (self.weighted_mean(x[:, : self.num_coeff_kept]), self.weighted_mean_(x[:, self.num_coeff_kept :])), dim=-1
        )
