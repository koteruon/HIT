...
from hit.modeling.roi_heads.action_head.context_modeling import ContextMashing
...
###AVA 特別制定模型 start
@registry.ROI_ACTION_FEATURE_EXTRACTORS.register("2MLPFeatureExtractor_AVA")
class MLPFeatureExtractor_AVA(nn.Module):
    def __init__(self, config, dim_in):
        super(MLPFeatureExtractor_AVA, self).__init__()
        self.config = config
        head_cfg = config.MODEL.ROI_ACTION_HEAD
        ...
        for l in [self.fc1, self.fc2]:
            ...
            nn.init.constant_(l.bias, 0)
        self.dim_out = representation_size
        self.context_model = ContextMashing()
    ...
    def forward(
        ...
        if part_forward == 1:
            person_pooled = cat([box.get_field("pooled_feature") for box in proposals])
            context_interaction = cat([box.unsqueeze(0) for box in context])
        else:
            # print(slow_features.shape, fast_features.shape)
            context_feats = torch.cat(
                [slow_features.mean(dim=2, keepdim=True), fast_features.mean(dim=2, keepdim=True)], dim=1
            )
            ...
            data = {
                "features": context_feats,
                "rois": [p.bbox.shape[0] for p in proposals],
                "num_rois": x.shape[0],
                "pooled": x,
            }
            person_pooled = self.context_model(data)
            context_interaction = torch.rand(person_pooled.size(), device="cuda")

        if part_forward == 0:
            return None, person_pooled, context_interaction
        ...
        x_after = F.relu(self.fc1(x_after))
        x_after = F.relu(self.fc2(x_after))
        return x_after, person_pooled, context_interaction
