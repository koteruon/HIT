...
@registry.ACTION_DETECTOR.register("ActionDetector")
...
###AVA 特別制定模型 start
@registry.ACTION_DETECTOR.register("ActionDetector_AVA")
class ActionDetector_AVA(nn.Module):
    def __init__(self, cfg):
        super(ActionDetector_AVA, self).__init__()
        self.backbone = build_backbone(cfg)
        self.roi_heads = build_3d_roi_heads(cfg, self.backbone.dim_out)
...
def build_detection_model(cfg):
    func = registry.ACTION_DETECTOR[cfg.MODEL.ACTION_DETECTOR.STRUCTURE]
    return func(cfg)
