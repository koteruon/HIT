    ...
    def __init__(self, cfg, dim_in, dim_out, kernels, stride, padding, dilation=1, init_weight=None):
        ...
        if cfg.MODEL.BACKBONE.FROZEN_BN:
            ...
            self.bn = nn.BatchNorm3d(
                dim_out, eps=cfg.MODEL.BACKBONE.BN_EPSILON, momentum=cfg.MODEL.BACKBONE.BN_MOMENTUM
            )
...
class Bottleneck(nn.Module):
    def __init__(self, cfg, dim_in, dim_out, dim_inner, stride, dilation=1, use_temp_conv=1, temp_stride=1):
        ...
        self.conv3 = Conv3dBN(
            ...
            stride=(1, 1, 1),
            padding=0,
            init_weight=cfg.MODEL.BACKBONE.BN_INIT_GAMMA,
        )
...
class ResBlock(nn.Module):
    def __init__(
        self, cfg, dim_in, dim_out, dim_inner, stride, dilation=1, use_temp_conv=0, temp_stride=1, need_shortcut=False
    ):
        super(ResBlock, self).__init__()

        self.btnk = Bottleneck(
            cfg,
            dim_in,
            dim_out,
            dim_inner=dim_inner,
...
class ResNLBlock(nn.Module):
    def __init__(
        ...
        for idx in range(num_blocks):
            ...
            dim_in0 = (
                dim_in + int(dim_in * cfg.MODEL.BACKBONE.SLOWFAST.BETA * 2)
                if lateral and (idx == 0) and (cfg.MODEL.BACKBONE.SLOWFAST.LATERAL != "ttoc_sum")
                else dim_in
            )
            ...
            if idx % nonlocal_mod == nonlocal_mod - 1:
                ...
                nl_module = NLBlock(dim_in, dim_in, int(dim_in / 2), cfg.MODEL.BACKBONE.NONLOCAL, group=group_nonlocal)
