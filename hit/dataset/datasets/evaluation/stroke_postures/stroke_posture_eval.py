...
def save_stroke_postures_results(dataset, predictions, output_folder, logger):
    logger.info("Preparing results for stroke postures format")
    ava_results = prepare_for_stroke_postures_detection(predictions, dataset)
    logger.info("Evaluating predictions")
    with tempfile.NamedTemporaryFile() as f:
        file_path = f.name
        if output_folder:
            file_path = os.path.join(output_folder, "result_stroke_postures.csv")
            top1_action_by_frame_confusion_matrix_file_path = os.path.join(
                output_folder, "result_top1_action_by_frame_confusion_matrix_stroke_postures.csv"
            )
            top1_action_by_video_confusion_matrix_file_path = os.path.join(
                output_folder, "result_top1_action_by_video_confusion_matrix_stroke_postures.csv"
            )
