...
def save_nw_ucla_results(dataset, predictions, output_folder, logger):
    logger.info("Preparing results for nw_ucla format")
    ava_results = prepare_for_nw_ucla_detection(predictions, dataset)
    logger.info("Evaluating predictions")
    with tempfile.NamedTemporaryFile() as f:
        file_path = f.name
        if output_folder:
            file_path = os.path.join(output_folder, "result_nw_ucla.csv")
            top1_action_by_frame_confusion_matrix_file_path = os.path.join(
                output_folder, "result_top1_action_by_frame_confusion_matrix_nw_ucla.csv"
            )
            top1_action_by_video_confusion_matrix_file_path = os.path.join(
                output_folder, "result_top1_action_by_video_confusion_matrix_nw_ucla.csv"
            )
...
def prepare_for_nw_ucla_detection(predictions, dataset):
    ava_results = {}
...
def write_csv(ava_results, csv_result_file, logger):
    ...
    dict_data = testlist_to_dict(csv_result_file.split("/")[0] + "/nw_ucla/annotations")
    start = time.time()
...
def write_top1_action_by_frame_confusion_matrix_csv(ava_results, csv_result_file, logger, dataset):
    ...
    with open(csv_result_file, "w") as csv_file:
        ...
        for i in range(num_classes):
            ...
            row_with_padding = [f"{val:>4}" for val in row]
