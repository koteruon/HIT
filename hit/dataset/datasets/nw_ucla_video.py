...
class NpBoxDict(object):
    def generate_range(self, num, frame_span, movie_map):
        ...
        max_val = min(movie_map[base], tail_digit + right_span)
    ...
    def __init__(self, id_to_box_dict, key_list=None, value_types=[], clip2ann=None, frame_span=None):
        ...
        if "video_keypoints" in value_fields:
            movie_map = {}
            for k in key_list:
                group_id = (k // 100000) * 100000
                tail_digit = k % 100000
                if group_id in movie_map:
                    if movie_map[group_id] < tail_digit:
                        movie_map[group_id] = tail_digit
                else:
                    movie_map[group_id] = tail_digit
        for k in key_list:
            if "video_keypoints" in value_fields:
                ...
                video_key_list = self.generate_range(k, frame_span, movie_map)
    ...
    def __init__(
        ...
        if "annotations" in json_dict:
            for ann in json_dict["annotations"]:
                ...
                one_hot = np.zeros(10 + 1, dtype=np.bool_)
    ...
    def _decode_video_data(self, dirname, timestamp):
        ...
        while cur_t <= folder_list.shape[0]:
            if (cur_t - timestamp) > right_span:
                break
            ## JHMDB
            video_path = os.path.join(video_folder, "{}.jpg".format(str(cur_t).zfill(5)))
            ## AVA
